...
By representing graphs in an adjacency matrix it is possible to observe special patterns and reveal dependencies which might not be seen in the graph per se. By combining the benefits of both the matrix representation and the graph itself, a very powerful approach of graph analyzing may be achieved. 
...
\cleardoublepage
\include{survey-basics}


\cleardoublepage
\include{survey-scalability}


\cleardoublepage
\include{survey-celltypes}

\cleardoublepage
\include{survey-reordering}

\cleardoublepage
\include{survey-headers}
...
All in all, no program can be recommended in general for any application. It is therefore best to consider the type and size of the data before choosing a matrix visualization tool.
